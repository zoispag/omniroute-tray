...
- `icons.js` — provider brand marks (Claude/OpenAI/Gemini) as inline SVG, extracted from `@lobehub/icons` (MIT) since the app is offline (CSP forbids remote/CDN assets). Regenerate by extracting `d`/`fill`/`viewBox` from `@lobehub/icons/es/{Claude,OpenAI,Gemini}/components/{Color,Mono}.js`. Everything else comes from the server at runtime (see `provider_icons.rs`).
