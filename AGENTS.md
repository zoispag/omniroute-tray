...
- `icons.js` — provider brand marks (Claude/Codex/Antigravity/GitHub Copilot) as inline SVG, extracted from `@lobehub/icons` (MIT; a copy sits in the OmniRoute prefix at `node_modules/@lobehub/icons/es/<Name>/components/{Color,Mono}.js`) since the app is offline (CSP forbids remote/CDN assets). **GitHub Copilot must stay bundled**: OmniRoute reports the connection as provider `github`, the server has no `github.svg`, and its `copilot.svg` is *Microsoft* Copilot — aliasing to it shows the wrong product. Everything else comes from the server at runtime (see `provider_icons.rs`).
