...
- `provider_icons.rs` — provider brand marks from the local server's **`GET /providers/<id>.svg`** (no auth; OmniRoute's dashboard assets). Id resolution mirrors OmniRoute's `ProviderIcon.tsx` aliases (`opencode-go`→`opencode`, `qwen-cloud-token-plan`→`qwencloud`, …) then strips trailing `-segments` (`kimi-coding`→`kimi`), except for user-defined `openai-compatible-<uuid>`/`anthropic-compatible-<uuid>` nodes, which must not borrow the OpenAI/Anthropic mark and stay on the letter badge. `get_provider_icon` caches per provider in `AppState`, keyed to `active_version` (a restart/update/adopted daemon on another version drops the whole cache, misses included); a miss (server answered 404 for every candidate) is cached, an unreachable server is returned as an error so the popover retries. The webview CSP forbids loading the SVGs directly, hence the round-trip through Rust.
- `apikey.rs` — resolves OmniRoute API key silently: `.env` → **shared `storage.sqlite`** (read-only `rusqlite`, table `api_keys`; a `manage`/`admin`-scoped key is preferred over the most recently used one). **No Keychain by default** — the Keychain path triggered a macOS password prompt on every rebuild (ad-hoc signature changes) and was dropped; Keychain helpers remain `#[allow(dead_code)]` for a future mint case.
- `omniauth.rs` — `Credentials { api_key, cli_token }` attached to every request against the local server. **The Bearer key alone is not enough** once the dashboard has a password: every endpoint the popover reads except `/api/monitoring/health` is a MANAGEMENT route, and the default key is inference-only (`self:usage`), so the server answers `403 Invalid management token` (#42). The tray therefore also sends `x-omniroute-cli-token`, the loopback token OmniRoute's own CLI uses: `HMAC-SHA256(key = IOPlatformUUID lower-cased, msg = "omniroute-cli-auth-v1")` hex (mirrors `node-machine-id` + `src/lib/machineToken.ts`; `OMNIROUTE_CLI_TOKEN`/`OMNIROUTE_CLI_SALT` in `~/.omniroute/.env` are honoured). The server only accepts it from a loopback peer. Credentials are re-resolved lazily while the key is missing — a fresh install mints its first key after the server starts, i.e. after bootstrap already looked.
- `spaces.rs` — the popover follows the active space (#58). An AppKit window is bound to the space it was created on, and the popover is built once at launch, so a tray click from a full-screen app or another desktop showed nothing (or pulled the user back). `follow_active_space` ORs `CanJoinAllSpaces` (tao exposes only this one, via `set_visible_on_all_workspaces`) and `FullScreenAuxiliary` (draws over another app's full-screen space) into the window's `collectionBehavior`, clearing the members those two are mutually exclusive with (`MoveToActiveSpace`, `FullScreenPrimary`, `FullScreenNone`). Must be re-applied per window *object* — the flags survive hide/show but not `recreate_popover`.
- `traymenu.rs` — **macOS 27 click workaround (#54), temporary.** `tray-icon` 0.24.x attaches the tray's `NSMenu` to the `NSStatusItem` for its whole life, and macOS 27 stops forwarding clicks to a status item that owns one (TN3212; tauri-apps/tray-icon#355) — so `TrayIconEvent::Click` never fires, `show_menu_on_left_click(false)` is ignored, and the popover (the only UI) is unreachable. Upstream fixed it in tray-icon 0.25.1 by attaching the menu only while presenting it, but `tauri` 2.x pins `tray-icon = "0.24"` and there is no 0.24 backport, so the fix cannot be pulled in. `detached()` (macOS >= 27, or the `OMNIROUTE_TRAY_DETACHED_MENU` override so the path can be exercised on older macOS) makes `lib.rs` build the tray with **no** menu — that is what restores the click events — and `present()` pops the menu at the pointer on right-click instead. It also drives the status item highlight by hand, because `tray-icon` lights the item on any mouse-down but only clears it from the *left* `mouseUp:` handler, assuming AppKit's own menu presentation restored it. **Delete the module and the `detached_menu` branch once tauri depends on tray-icon 0.25** — the native path is better in every way.
...
- `main.js` — popover render loop (self-scheduling `setTimeout`, not `setInterval`, to avoid poll pileup). `renderRateLimits()` fetches + caches into `rateLimitCache`; `paintRateLimits()` re-renders instantly (used by the %left/%used toggle and per-account hide, so they don't refetch). Per-account hide persists in `localStorage` (`hiddenAccounts`, keyed `provider/account`).
  - **Hidden accounts must stay restorable (#52).** Settings → Accounts lists `settingsAccounts()` = live accounts ∪ hidden keys the server no longer reports (tagged "not reported"), plus a "Show all" reset; otherwise an account hidden and then signed out upstream could never be re-enabled. Usage rows show `accountLabel()`: an account named `main`/`default`/`<provider>` is labelled with the provider name, since every OAuth-style provider calls its account `main`.
  - **Provider marks**: `providerBadge()` uses the bundled `PROVIDER_ICONS` first, then marks fetched via `get_provider_icon` (`remoteIcons` cache, `normalizeSvg()` reduces the mark to an allowlist of inert SVG elements/attributes (no `<a>`/`<image>`/`<use>` with external hrefs, no animation, no `feImage`, only presentation properties in `style`; the server on :20128 is adopted, so its assets are untrusted), prefixes ids/scopes `<style>` rules per provider so inlined marks cannot recolour each other, and turns monochrome marks into `currentColor` so they survive both themes), else a lettered badge on a per-provider hue. Never the old identical `●` — that is what made two `main` rows indistinguishable.
- `icons.js` — provider brand marks (Claude/Codex/Antigravity/GitHub Copilot/OpenCode) as inline SVG, extracted from `@lobehub/icons` (MIT; a copy sits in the OmniRoute prefix at `node_modules/@lobehub/icons/es/<Name>/components/{Color,Mono}.js`) since the app is offline (CSP forbids remote/CDN assets). **GitHub Copilot must stay bundled**: OmniRoute reports the connection as provider `github`, the server has no `github.svg`, and its `copilot.svg` is *Microsoft* Copilot — aliasing to it shows the wrong product. **OpenCode is bundled too**: the server's `opencode.svg` is a 234×42 wordmark, and `normalizeSvg` rejects any mark wider/taller than 2:1 in favour of the letter badge. Everything else comes from the server at runtime (see `provider_icons.rs`).
...
- **API auth** (`src/server/authz/policies/management.ts`): management routes accept a dashboard session, a Bearer key **with the `manage`/`admin` scope**, or on loopback the machine-derived CLI token header (`x-omniroute-cli-token`, see `omniauth.rs`). With login off (no password / `requireLogin=false`) loopback requests pass anonymously. A Bearer without a management scope gets **403** (401 when no Bearer at all) — that is what a fresh install's default `self:usage` key hits the moment onboarding sets a password. The active key lives in `storage.sqlite` table `api_keys(key, is_active, revoked_at, last_used_at, scopes)` — the app reads it directly (DB is shared with the server). Route classes are in `src/shared/constants/publicApiRoutes.ts`: only `/api/monitoring/health` and `/api/usage/om-usage` of the endpoints we touch are PUBLIC.
