...
- `spaces.rs` — the popover follows the active space (#58). An AppKit window is bound to the space it was created on, and the popover is built once at launch, so a tray click from a full-screen app or another desktop showed nothing (or pulled the user back). `follow_active_space` ORs `CanJoinAllSpaces` (tao exposes only this one, via `set_visible_on_all_workspaces`) and `FullScreenAuxiliary` (draws over another app's full-screen space) into the window's `collectionBehavior`, clearing the mutually exclusive `FullScreenPrimary`. Must be re-applied per window *object* — the flags survive hide/show but not `recreate_popover`.
